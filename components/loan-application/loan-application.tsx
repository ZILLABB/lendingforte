...
export default function LoanApplicationPage() {
  ...
  const [formData, setFormData] = useState({
    ...
    idType: '',
    idNumber: '',
    idIssueDate: '', // Added missing field
    idExpirationDate: '',
    citizenshipStatus: '',
    employmentStatus: '',
    employerName: '',
    employerAddress: '', // Added missing field
    jobTitle: '',
    employmentLength: '',
    monthlyIncome: '',
    payFrequency: '',
    phoneNumber: '',
    workPhone: '', // Added missing field
    email: '',
    loanType: '',
    loanPurpose: '',
    requestedAmount: '',
    ...
    accountNumber: '',
    routingNumber: '',
    otherIncomeSource: '', // Added missing field
  ...
  // Add proper type for form errors
  ...
  // Use the router directly at the component level
  ...
  // Update handleSSNChange to handle SSN formatting
  ...
  // Update handleExistingLoanChange with proper typing
  const handleExistingLoanChange = (index: number, field: keyof typeof existingLoans[0], value: string) => {
    const updatedLoans = [...existingLoans];
  ...
  // Update handleChange to handle all input types properly
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const docRef = await addDoc(collection(getFirestore(), 'loanApplications'), {
        ...formData,
  ...
  // Move useEffect hooks inside component
  useEffect(() => {
  ...
  // 1. Add loading state for form submission
  const [isLoading, setIsLoading] = useState(false);

  // 2. Add form validation state
  const [isValid, setIsValid] = useState(false);

  // 4. Add error handling for location data
  useEffect(() => {
    try {
      if (formData?.country) {
      ...
      toast.error('Error loading location data. Please try again.');
    }
  }, [formData.state, formData.country]);

  // 5. Add error handling for cities data
  useEffect(() => {
    try {
      ...
      toast.error('Error loading city data. Please try again.');
    }
  }, [formData.state, formData.country]);

  // 6. Add form validation on step change
  useEffect(() => {
    setIsValid(validateCurrentStep(step));
  }, [step, formData]);

  // 7. Add cleanup for form data on unmount
  useEffect(() => {
    return () => {
      // Clear form data from localStorage if form is completed
      if (isSubmitted) {
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    ...
                    <div className="space-y-1">
                      ...
                      >
                        ...
                        {states.map((state: { isoCode: string; name: string }) => (
